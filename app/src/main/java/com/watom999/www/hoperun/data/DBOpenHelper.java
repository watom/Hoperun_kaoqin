...
public class DBOpenHelper extends SQLiteOpenHelper {
    ...
    //数据表的名称
    public static String TABLE_NAME = "UserInfo.db";
    //创建UserInfo数据表的SQL语句
    public static final String CREAT_USERINFO_TABLE = "creat table if not exists UserInfo(_id integer primary key autoincrement,user_name text,user_id text,user_sex text,user_password text,user_email_account text,user_email_password text,)";
    //数据表的版本号
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        //执行所有的创建SQL语句
        for (String expression : creatTableSQL
             ) {
            sqLiteDatabase.execSQL(expression);
        }
    ...
    public void onUpgrade(SQLiteDatabase sqLiteDatabase, int i, int i1) {
        //执行所有的更新SQL语句
        for (String expression : updateTableSQL
                ) {
            sqLiteDatabase.execSQL(expression);
        }
    }
